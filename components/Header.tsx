  ...
  return (
    <div className="flex items-center justify-between w-full h-full">
      <div className="flex items-center space-x-4">
        ...
        <div>
           <h1 className="text-sm font-bold tracking-tight uppercase text-zinc-100">{activeModel}</h1>
           <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-medium">Neural State: Optimal</p>
        </div>
      </div>
      
      <div className="flex items-center space-x-4">
        <div className="hidden sm:flex items-center bg-zinc-800/50 p-1 rounded-xl">
          {(['en', 'ur', 'ar'] as Language[]).map(lang => (
            <button
              key={lang}
              onClick={() => onSetLanguage(lang)}
              className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${language === lang ? 'bg-zinc-700 text-zinc-100 shadow-sm' : 'text-zinc-500 hover:text-zinc-400'}`}
            >
              {lang}
            </button>
          ))}
        ...
        <button 
          ...
          className="p-2.5 rounded-xl bg-zinc-800/50 text-zinc-400 hover:bg-zinc-700 transition-colors"
        >
